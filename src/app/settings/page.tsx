            ...
            <div className="flex-1 min-w-0 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="h-full overflow-y-auto p-6 lg:p-8">
                <div className="max-w-2xl">
                  ...
                  {activeTab === 'profile' && <ProfileSettings user={user} openUserProfile={openUserProfile} />}
                  {activeTab === 'plan' && (
                    <PlanSettings onUpgrade={() => setIsPricingModalOpen(true)} />
                  )}
                  {activeTab === 'notifications' && <NotificationSettings />}
                  {activeTab === 'security' && <SecuritySettings openUserProfile={openUserProfile} />}
                </div>
              </div>
            </div>
...
function ProfileSettings({ user, openUserProfile }: { user: any, openUserProfile: () => void }) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-6 pb-6 border-b border-slate-100">
        <div className="relative">
          <img 
            src={user?.imageUrl || `https://ui-avatars.com/api/?name=${userName}&background=0f172a&color=fff&size=128`}
            alt="Profile" 
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Update your profile picture via Clerk.</p>
        </div>
      </div>
...
function PlanSettings({ onUpgrade }: { onUpgrade: () => void }) {
  return (
    <div className="space-y-8">
      {/* Current Plan */}
      <div className="p-4 rounded-xl border bg-[#D4E815]/10 border-[#D4E815]/30 space-y-4">
        <div className="flex items-start justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold text-[#1A1D21]">Free Plan</span>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide border bg-[#D4E815]/20 text-[#1A1D21] border-[#D4E815]/40">
                Active
              </span>
            </div>
...
function SecuritySettings({ openUserProfile }: { openUserProfile: () => void }) {
  return (
    <div className="space-y-6">
      <div className="space-y-4">
        <h3 className="text-sm font-bold text-slate-900">Password & Security</h3>
        <p className="text-sm text-slate-500">
          Manage your password and security settings through Clerk's secure portal.
        </p>
        <div className="pt-2">
          <button 
            onClick={openUserProfile}
