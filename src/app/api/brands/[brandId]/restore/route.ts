...
import { restoreManagedBrand } from '@/lib/brand-locations/management-postgres';
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const brand = await restoreManagedBrand(accountId, brandId);
    return NextResponse.json({ brand });
  } catch (error) {
    return managementErrorResponse(error, 'Failed to restore brand');
  }
}
